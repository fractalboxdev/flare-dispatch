 ...
 * run-scale) narrows the idle window at negligible cost. It buys no durability:
 * container disk is ephemeral on every path (ADR-0001 rule 5), so a run that
 * needs its checkout between steps re-establishes it with `ensureWorkspace`.
