...
// repo's `infra/maintenance-loop/declined.jsonl` is never proposed again; a
...
const renderPrBody = (args: RenderArgs & { message: string }): string =>
  [
    ...
    "",
    "> Closing this unmerged suppresses every key below for 30 days. To suppress one permanently, add its key to `infra/maintenance-loop/declined.jsonl` with a reason.",
    "",
