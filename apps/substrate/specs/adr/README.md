...
- **No status here has ever changed.** Measured across all fifteen ADRs in both sets
  ...
  so nothing ever has.
- **Status does not track implementation, in either direction.** Seven `Proposed` records are
  `shipped` — including ADR-0004 and ADR-0005, the ticket gate and the egress floor, both wired and
  heavily tested — while `Accepted` ADR-0002 is `partial`. If the root convention governed here,
  "do not implement against a proposed record's interfaces" would have been broken seven times.
...
| [0002](0002-substrate-inside-the-flare-dispatch-monorepo.md) | Inside the flare-dispatch monorepo, no name of its own | Accepted | partial — the lint rule forbidding imports from substrate internals does not exist (`.oxlintrc.json` has no rules) |
...
| [0011](0011-sdk-pin-as-security-surface.md) | The sandbox SDK pin is a security surface | Proposed | shipped — the enforcing test catches image/package skew, not a bump; no `@cloudflare/containers` pin and no bot config to disable |
