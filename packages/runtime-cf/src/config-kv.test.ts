...
describe("ConfigKvLive", () => {
  ...
  describe("Worker env fallback (credentials before KV)", () => {
    const runWith = <A>(
      ...
      Effect.runPromise(
        effect.pipe(
          Effect.provide(makeConfigKvLive(bindings.kv, overrides, envFallback)),
        ),
      );

    it("resolves secret/FOO from envFallback bare name FOO when KV unset", async () => {
      const value = await runWith(
        (name) => (name === "CLOUDFLARE_API_TOKEN" ? "tok_from_worker" : undefined),
        Effect.flatMap(Config, (c) => c.get("secret/CLOUDFLARE_API_TOKEN")),
      );
      expect(value).toBe("tok_from_worker");
    });

    it("Worker env wins over a stale KV value", async () => {
      await bindings.kv.put("secret/CLOUDFLARE_API_TOKEN", "tok_from_kv");
      const value = await runWith(
        (name) => (name === "CLOUDFLARE_API_TOKEN" ? "tok_from_worker" : undefined),
        Effect.flatMap(Config, (c) => c.get("secret/CLOUDFLARE_API_TOKEN")),
      );
    ...
    it("falls through to KV when envFallback returns undefined", async () => {
      await bindings.kv.put("secret/CLOUDFLARE_ACCOUNT_ID", "acct_from_kv");
      const value = await runWith(
        () => undefined,
        Effect.flatMap(Config, (c) => c.get("secret/CLOUDFLARE_ACCOUNT_ID")),
      );
      expect(value).toBe("acct_from_kv");
    });

    it("empty-string envFallback falls through to KV", async () => {
      await bindings.kv.put("secret/FOO", "from_kv");
      const value = await runWith(
        () => "",
        Effect.flatMap(Config, (c) => c.get("secret/FOO")),
      );
      expect(value).toBe("from_kv");
    });

    it("overrides still win over Worker env", async () => {
      const value = await runWith(
        () => "from_worker",
        Effect.flatMap(Config, (c) => c.get("secret/FOO")),
        { "secret/FOO": "from_override" },
      );
      expect(value).toBe("from_override");
    });

    it("bare key (no slash) looks up envFallback by the full key", async () => {
      const value = await runWith(
        (name) => (name === "CLOUDFLARE_ACCOUNT_ID" ? "acct_var" : undefined),
        Effect.flatMap(Config, (c) => c.get("CLOUDFLARE_ACCOUNT_ID")),
      );
      expect(value).toBe("acct_var");
    });
  });
});
