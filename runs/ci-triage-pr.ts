...
// keeps ONE rolling draft pull request carrying the latest triage write-up
...
// confidence problem); the value is a single deduped, model-written triage of
// what's red, filed where the team will see it. A red day refreshes the PR in
// place; a green day closes it. A PR a human has pushed to is never
...
/**
 * The ONE head branch for the triage PR. `openDraftPullRequest` is idempotent
 * on the head branch, so a stable name keeps a single rolling PR; a
 * date-keyed name opens a new PR every red day.
 */
export const HEAD_BRANCH = "flare-dispatch/ci-triage";

/** Posted on the rolling PR when a fire finds nothing red. */
const CLOSE_COMMENT = `Closed by \`flare-dispatch/ci-triage-pr\`: the latest scan finds no CI failures, failed deploys, or signals in the window.`;
...
const Output = Schema.Struct({
  ...
  /** `true` when a green fire closed the rolling PR. */
  prClosed: Schema.Boolean,
  /** `true` when human commits on the rolling PR kept this fire from touching it. */
  prHeld: Schema.Boolean,
  ...
  run: (input) =>
    Effect.gen(function* () {
      ...
      if (actionFailures.length === 0 && deployFailures.length === 0 && signals.length === 0) {
        const closed = yield* step("close-pr", () =>
        ...
        return {
          ...empty,
          prClosed: closed.closed,
          prHeld: !closed.closed && closed.reason === "human-owned",
          prNumber: closed.number ?? 0,
        };
      }
      ...
      // 5. Open/update the single rolling triage draft PR.
      ...
      if (result.skipped) {
        yield* io.log(
          "info",
          `ci-triage-pr: ${HEAD_BRANCH} on ${reportRepo} carries human commits; left untouched`,
        );
        return {
